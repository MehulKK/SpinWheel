...
import com.macv.achivement.utils.BootstrapBrand;
import com.macv.achivement.utils.BootstrapBrandView;
import com.macv.achivement.utils.BootstrapSizeView;
import com.macv.achivement.utils.ColorUtils;
import com.macv.achivement.utils.DefaultBootstrapBrand;
import com.macv.achivement.utils.DefaultBootstrapSize;
import com.macv.achivement.utils.DimenUtils;
import com.macv.achivement.utils.ProgressView;
import com.macv.achivement.utils.RoundableView;
...
public class AchievementProgressBar extends View implements ProgressView, BootstrapBrandView,
        RoundableView, BootstrapSizeView, Animator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {

    private static final String TAG = "com.macv.AwesomeTextView";

    private static final long UPDATE_ANIM_MS = 2000;
    private static final int STRIPE_ALPHA = 150;
    private static final long STRIPE_CYCLE_MS = 1500;

    private Paint progressPaint;
    private Paint transStrokePaint;
    ...
    private void initialise(AttributeSet attrs) {
        ...
        transStrokePaint = new Paint();
        transStrokePaint.setStyle(Paint.Style.STROKE);
        transStrokePaint.setAntiAlias(true);
        transStrokePaint.setStrokeWidth(8);
        transStrokePaint.setColor(Color.YELLOW);
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        if(valueUpdateListener != null)
            valueUpdateListener.value(drawnProgress);
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (striped) { // draw a regular striped bar
            ...
            //.drawRect(0, 0, lineEnd, h, transStrokePaint);
        }

        float corners = rounded ? h / 2 : 0;
        progressCanvas.drawRect(lineEnd, 0, w, h, bgPaint); // draw bg
        final RectF frame = new RectF(0, 0, progressBitmap.getWidth(), progressBitmap.getHeight());
        progressCanvas.drawRoundRect(frame, corners, corners, transStrokePaint); // draw bg
        //progressCanvas.drawRect(0, 0, w, h, transStrokePaint); // draw stroke
...
//        final Rect frameRight = new Rect(bitmap.getWidth() /2, bitmap.getHeight(), bitmap.getWidth(), bitmap.getHeight());
    ...
    private void updateBootstrapState(int color, int strokeColor, int backgroundColor) {
        progressPaint.setColor(color);
        transStrokePaint.setColor(strokeColor);
    ...
    @Override
    public void setProgressbarColor(BootstrapBrand bootstrapBrand, int color, int strokeColor, int backgroundColor) {
        this.bootstrapBrand = bootstrapBrand;
        updateBootstrapState(color, strokeColor, backgroundColor);
    }
