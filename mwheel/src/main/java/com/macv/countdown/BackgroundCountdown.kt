...
internal class BackgroundCountdown : BaseCountdown() {
    ...
    override fun initStyleAttr(context: Context, ta: TypedArray) {
        ...
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.CountdownView_timeBgDivisionLineSize, dp2px(context, DEFAULT_TIME_BG_DIVISION_LINE_SIZE))
        mTimeBgSize = ta.getDimension(R.styleable.CountdownView_timeBgSize, 0f)
        mDefSetTimeBgSize = mTimeBgSize
        mTimeBgBorderSize = ta.getDimension(R.styleable.CountdownView_timeBgBorderSize, dp2px(context, DEFAULT_TIME_BG_BORDER_SIZE))
    ...
    override fun initTimeTextBaseInfo() {
        ...
        if (mDefSetTimeBgSize == 0f || mTimeBgSize < mTimeTextWidth) {
            mTimeBgSize = mTimeTextWidth + dp2px(mContext, 2f) * 4
        }
    }
    ...
    private fun initTimeBgRect(topPaddingSize: Float) {
        ...
        if (isShowDay) {
            ...
            if (isShowTimeBgBorder) {
                mDayBgBorderRectF = RectF(mLeftPaddingSize, topPaddingSize, mLeftPaddingSize + mDayTimeBgWidth + mTimeBgBorderSize * 2, topPaddingSize + mTimeBgSize + mTimeBgBorderSize * 2)
                mDayBgRectF = RectF(mLeftPaddingSize + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, mLeftPaddingSize + mDayTimeBgWidth + mTimeBgBorderSize, topPaddingSize + mTimeBgSize + mTimeBgBorderSize)
            } else {
                mDayBgRectF = RectF(mLeftPaddingSize, topPaddingSize, mLeftPaddingSize + mDayTimeBgWidth, topPaddingSize + mTimeBgSize)
            }
            // hour left point
        ...
        if (isShowHour) {
            ...
            if (isShowTimeBgBorder) {
                mHourBgBorderRectF = RectF(mHourLeft, topPaddingSize, mHourLeft + mTimeBgSize + mTimeBgBorderSize * 2, topPaddingSize + mTimeBgSize + mTimeBgBorderSize * 2)
                mHourBgRectF = RectF(mHourLeft + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, mHourLeft + mTimeBgSize + mTimeBgBorderSize, topPaddingSize + mTimeBgSize + mTimeBgBorderSize)
            } else {
                mHourBgRectF = RectF(mHourLeft, topPaddingSize, mHourLeft + mTimeBgSize, topPaddingSize + mTimeBgSize)
            }
            // minute left point
        ...
        if (isShowMinute) {
            ...
            if (isShowTimeBgBorder) {
                mMinuteBgBorderRectF = RectF(mMinuteLeft, topPaddingSize, mMinuteLeft + mTimeBgSize + mTimeBgBorderSize * 2, topPaddingSize + mTimeBgSize + mTimeBgBorderSize * 2)
                mMinuteBgRectF = RectF(mMinuteLeft + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, mMinuteLeft + mTimeBgSize + mTimeBgBorderSize, topPaddingSize + mTimeBgSize + mTimeBgBorderSize)
            } else {
                mMinuteBgRectF = RectF(mMinuteLeft, topPaddingSize, mMinuteLeft + mTimeBgSize, topPaddingSize + mTimeBgSize)
            }
            // second left point
        ...
        if (isShowSecond) {
            ...
            if (isShowTimeBgBorder) {
                mSecondBgBorderRectF = RectF(mSecondLeft, topPaddingSize, mSecondLeft + mTimeBgSize + mTimeBgBorderSize * 2, topPaddingSize + mTimeBgSize + mTimeBgBorderSize * 2)
                mSecondBgRectF = RectF(mSecondLeft + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, mSecondLeft + mTimeBgSize + mTimeBgBorderSize, topPaddingSize + mTimeBgSize + mTimeBgBorderSize)
            } else {
                mSecondBgRectF = RectF(mSecondLeft, topPaddingSize, mSecondLeft + mTimeBgSize, topPaddingSize + mTimeBgSize)
            }
            if (isShowMillisecond) {
                // millisecond left point
                val mMillisecondLeft = mSecondLeft + mTimeBgSize + mSuffixSecondTextWidth + mSuffixSecondLeftMargin + mSuffixSecondRightMargin + mTimeBgBorderSize * 2

                // initialize millisecond background border rectF
                if (isShowTimeBgBorder) {
                    mMillisecondBgBorderRectF = RectF(mMillisecondLeft, topPaddingSize, mMillisecondLeft + mTimeBgSize + mTimeBgBorderSize * 2, topPaddingSize + mTimeBgSize + mTimeBgBorderSize * 2)
                    mMillisecondBgRectF = RectF(mMillisecondLeft + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, mMillisecondLeft + mTimeBgSize + mTimeBgBorderSize, topPaddingSize + mTimeBgSize + mTimeBgBorderSize)
                } else {
                    mMillisecondBgRectF = RectF(mMillisecondLeft, topPaddingSize, mMillisecondLeft + mTimeBgSize, topPaddingSize + mTimeBgSize)
                }
            }
            if (!isInitHasBackgroundTextBaseY) {
    ...
    private fun initHasBackgroundTextBaseY(rectF: RectF) {
        ...
        mTimeBgDivisionLineYPos = rectF.centerY() + if (mTimeBgDivisionLineSize == dp2px(mContext, DEFAULT_TIME_BG_DIVISION_LINE_SIZE)) mTimeBgDivisionLineSize else mTimeBgDivisionLineSize / 2
    }
     ...
     */
    private fun initTimeTextBaselineAndTimeBgTopPadding(viewHeight: Int, viewPaddingTop: Int, viewPaddingBottom: Int, contentAllHeight: Int): Float {
        val topPaddingSize: Float
        topPaddingSize = if (viewPaddingTop == viewPaddingBottom) {
            // center
        ...
        get() {
            ...
            if (isShowDay) {
                if (isDayLargeNinetyNine) {
                    val rect = Rect()
                    val tempDay = java.lang.String.valueOf(mDay)
                    mTimeTextPaint!!.getTextBounds(tempDay, 0, tempDay.length, rect)
                    mDayTimeBgWidth = rect.width() + dp2px(mContext, 2f) * 4
    ...
    override fun onMeasure(v: View, viewWidth: Int, viewHeight: Int, allContentWidth: Int, allContentHeight: Int) {
        val retTopPaddingSize = initTimeTextBaselineAndTimeBgTopPadding(viewHeight, v.paddingTop, v.paddingBottom, allContentHeight)
    ...
    fun setTimeBgSize(size: Float) {
        mTimeBgSize = dp2px(mContext, size)
    }
    ...
    fun setTimeBgRadius(radius: Float) {
        mTimeBgRadius = dp2px(mContext, radius)
    }
    ...
    fun setTimeBgDivisionLineSize(size: Float) {
        mTimeBgDivisionLineSize = dp2px(mContext, size)
        if (null != mTimeBgDivisionLinePaint) {
            mTimeBgDivisionLinePaint!!.strokeWidth = mTimeBgDivisionLineSize
        }
    }
    ...
    fun setTimeBgBorderSize(size: Float) {
        mTimeBgBorderSize = dp2px(mContext, size)
    ...
    fun setTimeBgBorderRadius(size: Float) {
        mTimeBgBorderRadius = dp2px(mContext, size)
    }
